...
export async function createSplToken(params: {
  ...
  signTransaction: (tx: Transaction) => Promise<Transaction>, // ✅ new
}) {
  const endpoint = params.cluster === 'devnet'
  ...
  return { mint: mintKeypair.publicKey.toBase58(), ata: ataPubkey.toBase58(), signature: sig };
}